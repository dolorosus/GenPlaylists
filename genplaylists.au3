...
Func genPlaylist($sSrcPath = "", $sPlaylistFldr = "")

	Local Const $bGenExtented = False
	Local Const $Contributing_artists = 13
	Local Const $Album = 14
	Local Const $Year = 15
	Local Const $Genre = 16
	Local Const $Conductors = 17
	Local Const $Authors = 20
	Local Const $Title = 21
	Local Const $Length = 27
	...
	Local $sPlaylist, $sArtist, $sPlDrv, $sPlPath, $sPlName, $sPlExt
	Local $fh

	Local $aFinfo


	Local $sStarttime, $sEndtime, $iStarttime, $iEndtime
	Local $aTime, $iTime

	While $sSrcPath = ""
		$sSrcPath = FileSelectFolder("Ordner suchen", "")
	...
	Switch @error
		...
		Case 9
			...
			$aFldrlist[1] = "."
		Case Else
			Msg(sf("Ordnerliste konnte nicht erstellt werden. Errorcode_:%d\n", @error))
			Exit 1
	EndSwitch
	...
	For $i = 1 To $aFldrlist[0]
		...
		;
		$sPlaylist = StringReplace($aFldrlist[$i] & ".m3u", "\.m3u", ".m3u")

		Msg(sf("%s\n", $sPlaylist))
		$fh = FileOpen($sPlaylist, $FO_OVERWRITE)


		$aFilelist = _FileListToArrayRec($aFldrlist[$i], "*", $FLTAR_FILES, $FLTAR_RECUR, Default, $FLTAR_FULLpath)

		If @error Then
			Msg(sf("Dateiliste konnte nicht erstellt werden. Errorcode_:%s\n", @error))
		Else
			;
			...
			If @error Then Msg(sf("Playliste %s konnte nicht sortiert werden. Errorcode_:%s\n", $sPlaylist, @error))
			;
			; Das Toyota Touch benötigt den #EXTM3U Eintrag,
			; obwohl extended Playlisten nicht verarbeitet werden können...
			;
			FileWriteLine($fh, "#EXTM3U")
;
; Jetzt die Dateien in die PLayliste schreiben
;
			For $j = 1 To $aFilelist[0]
				If StringRight($aFilelist[$j], 4) = ".mp3" Then
					;
					; Wenn eine Extended Playlist erzeugt werden soll
					;
					If $bGenExtented Then
						$aFinfo = _GetExtProperty($aFilelist[$j], -1)
						;_ArrayDisplay($aFinfo)
						If Not @error Then
							$aTime = StringSplit($aFinfo[$Length], ":")
							$iTime = 3600 * $aTime[1] + 60 * $aTime[2] + $aTime[3]
							FileWriteLine($fh,sf("#EXTINF:%d,%s - %s - %s\n", $iTime, $aFinfo[$Title], $aFinfo[$Contributing_artists], $aFinfo[$Album]))
						EndIf
					EndIf
					;
					; Eintrag in Playliste schreiben
					;
		...
		FileClose($fh)
				;
		; Playliste mit dem Namen des Interpreten
		; des ersten Titels als Präfix versehen
		;
		If IsArray($aFilelist) Then
			$sArtist = _GetExtProperty($aFilelist[1], $Contributing_artists)
			If @error Or $sArtist = "0" Then
				;
				; Abfrage ging fehl, nixtun
				;ConsoleWrite("ERROR " &$sArtist& @CRLF)
				;
			Else
				_PathSplit($sPlaylist, $sPlDrv, $sPlPath, $sPlName, $sPlExt)
				;
				; sollte der Interpret bereits im Namen der Playliste enthalten sein,
				; wird nicht umbenannt
				;
				If StringInStr($sPlName, $sArtist) > 0 Or (StringLen($sPlName) < 3) Then
					;
					; Nixtun
					;
				Else
					Msg(sf("Rename_:'%s' to '%s'\n", $sPlaylist, $sPlDrv & $sPlPath & $sArtist & "-" & $sPlName & $sPlExt))
					FileMove($sPlaylist, $sPlDrv & $sPlPath & $sArtist & "-" & $sPlName & $sPlExt)
				EndIf
			EndIf
		EndIf
		;
