...
Func _ID3DeleteFiles()
	...
	Local $aID3File = StringSplit($ID3Filenames, "|")
	For $i = 1 To $aID3File[0]
		If FileExists($aID3File[$i]) Then
			Local $ret = FileDelete($aID3File[$i])
...
Func _ID3v1Tag_ReadFromFile($Filename)
	...
	Local $ID3v1PlusID = BinaryToString(BinaryMid($ID3v1Plus_RawDataBinary, 1, 4))
...
;~ 		MsgBox(0,"$ID3v1PlusID",$ID3v1PlusID)
	...
	If $ID3v1_TagFound And Not $ID3v1Plus_TagFound Then
		Local $Version = _ID3v1Tag_GetVersion()
...
Func _ID3v1Tag_WriteToFile($sFilename)
	...
	;Write MP3 Data
	Local $hfile = FileOpen($sFilename, 16 + 1) ;Open for write force binary
	Local $Test = FileSetPos($hfile, $iFileSetPos, $FILE_END) ;Go to End of File (-128 if tag exists)
...
;~ 		MsgBox(0,"$iBytesRead",$iBytesRead & " of " & $iTagSize)
		Local $sFrameIndex = StringInStr($sID3v2_TAGINFO, $sFrameID)
		If $sFrameIndex == 0 Then
			$sID3v2_TAGINFO &= "|" & $sFrameID & ":" & "1"
		Else

			Local $sFrameIndexEnd = StringInStr(StringMid($sID3v2_TAGINFO, $sFrameIndex), ":")
			Local $iNumFrameID = 1 + Number(StringMid($sID3v2_TAGINFO, $sFrameIndex + $sFrameIndexEnd, 1))
			Local $sNewFrameIDString = $sFrameID & ":" & $iNumFrameID
...
;~ 	_ArrayDisplay($aFrameIDSplit)
	...
	Local $Firstpipe = StringInStr($FrameInfoCut, '|')
	Local $Lastpipe = StringInStr($FrameInfoCut, '|', -1, -1)
		...
		If $iReturnTypeFlag == 0 Then
			...
			If ($sFrameID == "TCON") Or ($sFrameID == "TCO") Then ;Content Type/Genre
				If StringMid($vFrameString, 1, 1) == "(" Then ;check if first char is "("
					Local $closeparindex = StringInStr($vFrameString, ")")
					Local $GenreID = StringMid($vFrameString, 2, $closeparindex - 1)
...
Func _h_ID3v2_GetFrameW000_WZZZ(ByRef $bFrameData)
	...
	Local $sFrameString = _h_ID3v2_DecodeTextToString($bText_Encoding, $bFrameData)
...
;~ 		MsgBox(0,"$sMIMEType",$sMIMEType)
	...
	Local $WriteError = FileWrite($PicFile_h, BinaryMid($bFrameData, $iBinaryStartIndex))
	FileClose($PicFile_h)
	;****************************************************************************************
...
;~ 			$14 Publisher/Studio logotype
	...
	Local $PicFile_h = FileOpen($sPictureFileName, 16) ;force binary
...
;~ 	MsgBox(0,"$MPEGFrameSyncHex",$MPEGFrameSyncHex)
	...
	Local $MPEGFrameSyncUint32 = Dec(StringReplace($MPEGFrameSyncHex, "0x", ""), 2)
...
;~ last frame, just before the ID3v1 tag (if any).
	...
	Local $APE_Footer = FileRead($hfile, 32)
	Local $APETAG_ID = BinaryToString(BinaryMid($APE_Footer, 1, 8))
...
Func _APEv2_RemoveTag($Filename)
	...
	Local $hTagFile = FileOpen($TagFilename, 2 + 8 + 16) ;erase all

	Local $hfile = FileOpen($Filename, 16) ;binary mode
