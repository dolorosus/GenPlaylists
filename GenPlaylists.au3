...
Func genPlaylist($sSrcPath = "")
	...
	For $i = 1 To $aFldrlist[0]
		...
		Else
			...
			If @error Then Msg(sf($csFlistSortErr, $sPlaylist, @error, @extended))
			;
		...
		If IsArray($aFilelist) Then
			...
			Else
				$aPathSplit = PathSplit($sPlaylist)
				;
				; ignore if the atrist is already part of the name
				;
				If StringInStr($aPathSplit[3], $sArtist) > 0 Or (StringLen($aPathSplit[3]) < 3) Then
				Else
					Msg(sf($csFmoveMsg, $sPlaylist, $aPathSplit[1] & $aPathSplit[2] & $sArtist & "-" & $aPathSplit[3] & $aPathSplit[4]))
					FileMove($sPlaylist, $aPathSplit[1] & $aPathSplit[2] & $sArtist & "-" & $aPathSplit[3] & $aPathSplit[4])
				EndIf
			EndIf
		EndIf
		;
